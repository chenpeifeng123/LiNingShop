...
export default class goodCar {
  ...
  checkHandler(e) {
    ...
    $.each($(".checks"), (index, ele) => {
      ele.checked = Array.from(this.ches).every(item => $(item).prop("checked"));
    });
    this.changeCheckBg();
    this.totalPrice();
    this.buyShopping();
  ...
  changeCheckBg() {
    $.each($("input[type*=che]"), (i, item) => {
      ...
      $(item).prop("checked") ? tNext.removeClass("ckss") : tNext.addClass("ckss");
    });
  }
  // 结算按钮
  buyShopping() {
  ...
  changeData(str, idx) {
    if (str === "del") {
      this.getAjax(this.data[idx].num, this.data[idx].rule, this.data[idx].good_id, str);
    } else if (str === "delAll") {
