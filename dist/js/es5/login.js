...
export default class Login {
  ...
  judge() {
    ...
    $(".form").on("input", "input", e => {
      this.ids = setTimeout(() => {
        ...
        if (!bool) {
          target.addClass("inpSuccess").removeClass("inpFail").next().text("× 格式错误").css("color", "#f33");
        } else {
          target.addClass("inpFail").removeClass("inpSuccess").next("span").text("√ 格式正确").css("color", "green");
        }
  ...
  testVerify(elem) {
    switch (elem.attr("id")) {
      ...
      case "vCode":
        return elem.val().toLowerCase() === this.vCode.options.code.toLowerCase();
      case "repeatPwd":
