export default class shopList {
  ...
  static setLeftList(data) {
    ...
    arr.forEach((elem, index) => {
      let divs = $("<div class='little'></div>").append("<p class='title'>" + data.title[index] + "</p>");
  ...
  setShopList(data) {
    data.forEach((item, index) => {
      ...
      let as = $(`<a  class='lbs' target="_blank">
              ...
              </a>`).appendTo(".listBox").attr("href", "./details.html?id=" + item.id).mouseleave(() => {
      ...
      $(`.imgBox>div[index=${index}]`).mouseover("img", function (e) {
        ...
        $(e.target).css("border-color", "#f33").siblings().css("border-color", "#b8b8b8");
      });
    });
  }
  // 分页
  setPages() {
    ...
    $("<span class='lfText'></span>").text(`显示 1-25(共${this.totalPages * 25}个)`).appendTo(".listFoot");
